...
//  E-Shopping
//
//  Created by Abdelrahman Samy on 15.03.2020.
...
class MUser {
    ...
    init(_dictionary: NSDictionary) {
        ...
        if let purchedsIds = _dictionary[kPURCHERDITEMIDS] {
            purchedItemIds = purchedsIds as! [String]
        } else {
            purchedItemIds = []
        }
    }
...
func saveUserToFirestore(mUser: MUser) {
    ...
    FirebaseRefrence(.User).document(mUser.objectId).setData(userDictionaryFrom(user: mUser) as! [String : Any]) { (error) in
...
func userDictionaryFrom(user: MUser) -> NSDictionary {
    ...
    return NSDictionary(objects: [user.objectId, user.email, user.firstName, user.lastName, user.fullName, user.fullAddress ?? "", user.onBoard, user.purchedItemIds], forKeys: [kOBJECTID as NSCopying, kEMAIL as NSCopying, kFIRSTNAME as NSCopying, kLASTNAME as NSCopying, kFULLNAME as NSCopying, kFULLADDRESS as NSCopying, kONBOARD as NSCopying, kPURCHERDITEMIDS as NSCopying])
}
...
func updateCurrentUserInFirestore(withValues: [String : Any], completion: @escaping (_ error: Error?) -> Void) {
    ...
    if let dictionary = UserDefaults.standard.object(forKey: kCURRENTUSER) {
        ...
        FirebaseRefrence(.User).document(MUser.currentId()).updateData(withValues) { (error) in
